...
fn arr_allclose<D: Dimension>(input: &Array<f32, D>, target: &Array<f32, D>) -> bool {
    assert_eq!(
        input.shape(),
        target.shape(),
        "\ninput had shape {:?}, but target had shape {:?}",
        input.shape(),
        target.shape()
    );
...
fn test_py_implementation_random_arrays_conv2d_transpose_stride2() {
    ...
    .unwrap();
    let conv_layer19 = TransposedConvolutionLayer::new(kernel19, 2, Padding::Same);
    ...
    .unwrap();
    let conv_layer20 = TransposedConvolutionLayer::new(kernel20, 2, Padding::Same);
    ...
    .unwrap();
    let conv_layer21 = TransposedConvolutionLayer::new(kernel21, 2, Padding::Same);
    ...
    .unwrap();
    let conv_layer22 = TransposedConvolutionLayer::new(kernel22, 2, Padding::Same);
    ...
    .unwrap();
    let conv_layer23 = TransposedConvolutionLayer::new(kernel23, 2, Padding::Same);
    ...
    .unwrap();
    let conv_layer24 = TransposedConvolutionLayer::new(kernel24, 2, Padding::Same);
